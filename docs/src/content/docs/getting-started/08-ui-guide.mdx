...
- **Settings**: Access theme preferences and initialized datasets.

### Keyboard Shortcuts

- `Alt+Q` — Focus the query editor
- `Alt+T` — Open time range selector
- `Shift+Enter` — Re-evaluate the pipeline (fast)
- `Cmd+Shift+Enter` (Mac) / `Ctrl+Shift+Enter` (Windows) — Full search
- `Cmd+Shift+C` (Mac) / `Ctrl+Shift+C` (Windows) — Copy share link
- `Cmd+N` (Mac) / `Ctrl+N` (Windows) — Toggle "until now" mode
...
Click the **Settings** icon (gear) in the search header to access:

### General Tab

- **Theme**: Choose your preferred color scheme (Midnight, Nord, Dracula, Catppuccin)
- **Profile**: Switch between different connector profiles

### Initialized Datasets

View and manage datasets that are loaded from your configured adapters.
...
For keyboard shortcuts reference, see [Keyboard Shortcuts](/getting-started/07-keyboard-shortcuts).
