...
<QueryPipeline
  steps={[
    {
      label: "Datasource",
      description: "@name — scope the query to a specific adapter (not yet implemented)",
      icon: "🔌",
    },
    {
      label: "Controller Parameters",
      description: "key=\"value\" pairs pushed to the backend adapter for early server-side filtering",
      icon: "🎛️",
    },
    {
      label: "Search Expression",
      description: "Free-text words or phrases matched against all fields in each record",
      icon: "🔍",
    },
    {
      label: "Pipeline Commands",
      description: "| where … | stats … | table … — transform and shape the results",
      icon: "⚙️",
    },
  ]}
/>
...
<QQLCode code={`controllerParam1="value1" controllerParam2="42" search terms | where ... | stats ... | table ...`} />
...
<ParamItem label={`key="value"`} type="equality">
  Include only records where `key` equals `value`.
</ParamItem>

<ParamItem label={`key!="value"`} type="exclusion">
  Exclude records where `key` equals `value`.
</ParamItem>

<ParamItem label="key=`pattern`" type="regex match">
  Include records where `key` matches the regex `pattern` (backtick-wrapped).
</ParamItem>

<ParamItem label="key!=`pattern`" type="regex exclusion">
