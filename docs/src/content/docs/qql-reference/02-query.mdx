...
- **Datasource** (optional): An `@name` token that scopes the query to a specific adapter.
...
When multiple adapters are configured, you can scope a query to a specific one by placing an `@datasource-name` token at the very start of the query:
...
```
@docker error | where level == "error"
```

Runs the query against the `docker` adapter only, filtering for records containing "error" where the `level` field equals `"error"`.

```
@my-loki-prod | where service == "api" | stats count() by status
```

Scopes the query to the `my-loki-prod` adapter and aggregates by status code.
