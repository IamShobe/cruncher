...
| `pattern` | string | A regular expression pattern (must be enclosed in backticks) |

## Returns

`boolean` — `true` if `string` matches the regex `pattern`, `false` otherwise.
...
```qql
eval is_http_status = match(status_code, `^[45][0-9]{2}$`)
```
Creates a boolean field `is_http_status` that is `true` if `status_code` is a 4xx or 5xx HTTP status.
...
```
Checks if a field looks like a UUID.