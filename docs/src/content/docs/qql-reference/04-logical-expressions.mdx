...
**Simple OR:**
<QQLCode code={`| where status == "error" || status == "warn"`} />

**Negation:**
<QQLCode code={`| where !(level == "info")`} />

**Grouping:**
<QQLCode code={`| where (status == "error" || status == "warn") && duration > 1000`} />

**With functions:**
<QQLCode code={`| where isNull(optional_field) || contains(message, "timeout")`} />
