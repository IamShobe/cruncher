...
<CommandCard
  name="stats"
  description="Perform aggregations over your data, optionally grouped by one or more fields."
  syntax="| stats [agg1(field1) as alias1, ...] [by groupField1, groupField2, ...]"
  example="| stats count() as total, avg(duration) as avg_dur by service"
  output="table"
/>
...
<QQLCode code={`| stats count() as total, avg(duration) as avg_duration, first(user) as first_user by status`} />

This counts all records, calculates the average `duration`, and returns the first `user` for each `status` group.

## See Also

- [`timechart`](/qql-reference/commands/timechart) — time-series aggregation (alternative to stats for trend visualization)
- [`sort`](/qql-reference/commands/sort) — sort aggregated results
- [`table`](/qql-reference/commands/table) — select and display specific columns
