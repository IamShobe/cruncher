...
<CommandCard
  name="regex"
  description="Extract values from a field using a regular expression and create new fields from captured groups."
  syntax='| regex [field="<FIELD_NAME>"] `<PATTERN>`'
  example={'| regex field="message" `(?<user>\\w+) logged in`'}
  output="logs"
/>
...
<QQLCode code={'| regex field="message" `(?<user>\\w+) logged in from (?<ip>\\d+\\.\\d+\\.\\d+\\.\\d+)`'} />
