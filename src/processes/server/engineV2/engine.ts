...
import {
    newBatchDoneMessage,
    newJobUpdatedMessage,
} from "src/processes/server/plugins_engine/router";
...
export class Engine {
    ...
    constructor(
        private messageSender: ResponseHandler,
    ...
    public async runQuery(
            ...
            (cacheKey: string, provider: InstanceRef) =>
                async (data: ProcessedData[]): Promise<void> => {
                    ...
                    try {
                        ...
                        console.error(`Error occurred while processing task ${taskId}:`, error);
                        // If an error occurs, we can mark the task as failed
                        queryTaskState.task.status = "failed";
                        queryTaskState.task.error = error instanceof Error ? error.message : "Unknown error";
                        messageSender.sendMessage(
                            newJobUpdatedMessage(taskId, queryTaskState.task.status, queryTaskState.task.error),
                        );
        ...
        Promise.allSettled(
            ...
            onTaskDone().then(() => {
                messageSender.sendMessage(newJobUpdatedMessage(taskId, task.status, queryTaskState.task.error));
                queryTaskState.finishedQuerying.signal();
            }).catch((error) => {
                console.error(`Error occurred while finalizing task ${taskId}:`, error);
                // If an error occurs, we can mark the task as failed
                task.status = "failed";
                queryTaskState.task.error = error.message;
                messageSender.sendMessage(
                    newJobUpdatedMessage(taskId, task.status, queryTaskState.task.error),
                );
                queryTaskState.finishedQuerying.signal(); // Signal that the task is done
            });
        });
    ...
    public cancelQuery(taskId: TaskRef): void {
        ...
        this.messageSender.sendMessage(
            newJobUpdatedMessage(taskId, taskState.task.status, taskState.task.error),
