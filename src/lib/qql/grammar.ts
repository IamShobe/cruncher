...
const RegexPattern = createToken({
  name: "RegexPattern",
  pattern:
    /`(?:[^\\`]|\\(?:[bfnrtv`\\/]|\.|u[0-9a-fA-F]{4}|\w|[\[\]\(\)\{\}]))*`/,
});

const RegexParamField = createToken({
...
const SearchOR = createToken({
  name: "SearchOR",
  pattern: matchKeywordOfSearch(/^OR/),
  longer_alt: Identifier,
  line_breaks: false,
});
const SearchAND = createToken({
  name: "SearchAND",
  pattern: matchKeywordOfSearch(/^AND/),
  longer_alt: Identifier,
  line_breaks: false,
});
const SearchParamNotEqual = createToken({
...
export class QQLParser extends EmbeddedActionsParser {
  ...
  private parathesisCalculateExpression = this.RULE(
    "parathesisCalculateExpression",
    (): CalcExpression => {
      this.CONSUME(OpenBrackets);
